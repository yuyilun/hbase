...
public class HBaseService {
	...
	public boolean createTable(String tableName,List<String> columnFamily) {
		...
		try {
			...
			columnFamily.forEach(cf ->{
				familyDescriptors.add(ColumnFamilyDescriptorBuilder
	...
	/**
	 * 自定义获取分区splitKeys（切分keys）
	 * 1、void createTable(HTableDescriptor desc, byte[][] splitKeys) 
	 * 这个函数的参数splitKeys是一个二维字节数据，行的最大数表示region划分数+1，列就表示region和region之间的行键。
	 * 底层也是调用异步方法，只不过多了超时时间。
	 * 2、void createTableAsync(HTableDescriptor desc, byte[][] splitKeys)  
	 * 3、void createTable(HTableDescriptor desc, byte[] startKey,byte[] endKey, int numRegions) 
	 * 是使用者指定表的“起始行键”、“末尾行键”和region的数量，这样系统自动给你划分region。根据的region数，来均分所有的行键。
	 * 这个方法的问题是如果你的表的行键不是连续的，那样的话就导致有些region的行键不会用到，有些region是全满的。
	 * 4、void createTable(HTableDescriptor desc)  第一个函数相对简单，就是创建一个表，这个表没有任何region。
	 * @param keys
	 * @return
	 */
	public byte[][] getSplitKeys(String[] keys){
	 ...
	 /**
	  * 按startKey、endKey、分区数，转变成16进制切分keys
	  * @param startKey
	  * @param endKey
	  * @param numRegions
	  * @return
    ...
    public static byte[][] getHexSplits(String startKey, String endKey, int numRegions) {
        ...
        BigInteger range = highestKey.subtract(lowestKey);//所有的范围
        BigInteger regionIncrement = range.divide(BigInteger.valueOf(numRegions));//每个分区的范围=所有的范围/分区数
        lowestKey = lowestKey.add(regionIncrement);//lowestKey = lowestKey + regionIncrement
        for(int i=0; i < numRegions-1;i++) {
            BigInteger key = lowestKey.add(regionIncrement.multiply(BigInteger.valueOf(i)));// key = lowestKey + regionIncrement*i 
            byte[] b = String.format("%016x", key).getBytes();//转变成原来的字节数组
            splits[i] = b;
        }
        return splits;
    }
    ...
    public Map<String,Map<String,String>> getResultScannerRowFilter(String tableName, String keyword){
        ...
        if(StringUtils.isNoneBlank(keyword)){
            Filter filter = new RowFilter(CompareOperator.GREATER_OR_EQUAL,new SubstringComparator(keyword));//GREATER_OR_EQUAL 的意思是 >=
            scan.setFilter(filter);
        }
